...
class SearchViewModel(
    ...
    fun vote(reportId: String, type: VoteType) {
        ...
        val currentVote = currentState.hotUserVotes[reportId]

        val newVote: VoteType? = when (currentVote) {
            type -> null
            else -> type
        }

        _uiState.update { s ->
            val updatedReports = s.hotReports.map { r ->
                if (r.id != reportId) return@map r

                var stillDanger = r.stillDangerCount
                var nowSafe = r.nowSafeCount

                when (currentVote) {
                    VoteType.STILL_DANGER -> stillDanger = (stillDanger - 1).coerceAtLeast(0)
                    VoteType.NOW_SAFE -> nowSafe = (nowSafe - 1).coerceAtLeast(0)
                    null -> Unit
                }

                when (newVote) {
                    VoteType.STILL_DANGER -> stillDanger += 1
                    VoteType.NOW_SAFE -> nowSafe += 1
                    null -> Unit
                }

                r.copy(stillDangerCount = stillDanger, nowSafeCount = nowSafe)
            }

            s.copy(
                hotReports = updatedReports,
                hotUserVotes = s.hotUserVotes.toMutableMap().apply {
                    if (newVote == null) remove(reportId) else put(reportId, newVote)
                }
            )
        }
        if (newVote != null) {
            viewModelScope.launch { hotRepo.vote(reportId, newVote) }
        }
    }

    fun setSelectedHotReport(id: String?) {
        _uiState.update { s ->
            s.copy(selectedHotReportId = id)
        }
    }
